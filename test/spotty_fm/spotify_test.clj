...
(deftest test-client-auth-wrapper

  (testing "Connecting to the live service to authenticate with the wrapper"
    (is (string? (fetch-client-auth-token (:clientid (:spotify config))
                                          (:secret (:spotify config)))) "Auth token should be a string"))

  (testing "Allowing environment variable override when it's set"

    (with-redefs-fn {#'environ.core/env {:spotify-auth-token "foo"}}

      #(is (= "foo" (fetch-client-auth-token (:clientid (:spotify config))
                                             (:secret (:spotify config))))))))
